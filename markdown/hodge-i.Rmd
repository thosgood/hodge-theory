...
::: {.rmenv #hodge-theory-i-principle-2.1 title="Principle 2.1" latex="{Principle 2.1}"}
...
When we have a resolution of singularities, we can often give a conjectural definition of $W$, whose validity follows from the Weil conjectures [@5] (cf. \ref{hodge-theory-i-6}).
...
\oldpage{427}
By \ref{hodge-theory-i-3}, we should expect for the cohomology groups $H^n(X(\CC),\ZZ)$ to carry natural mixed Hodge structures.
This is what we can prove (see [[Hodge Theory II, 3.2.5](hodge-theory-ii-3.2.5)] for the case where $X$ is smooth; the proof is algebraic, using classical Hodge theory [@6]).
...
Up to renumbering, the weight filtration of $H^\bullet(U,\QQ_l)$ is the abutment of \ref{hodge-theory-i-equation-6.2}:
$$
  \Gr_n^W(H^k(U,\QQ_l))
  = E_3^{2k-n,n-k}.
